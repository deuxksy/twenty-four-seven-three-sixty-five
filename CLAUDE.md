...
- `playbook-brla.yml`는 모든 role에 `tags: <role명>` 부여됨 → `--tags <role>` 선택 실행 가능 (예: homepage만 재배포). Hermes 단독 배포는 ACL fallback 포함 `playbook-hermes-only.yml` 사용
- SOPS `.env` 편집 후 라인마다 `export ` prefix 유지 확인 — 유실 시 셸 변수만 설정되고 자식 프로세스(ansible `lookup('env')`)에 전달 안 됨. `${VAR:+SET}`는 셸 변수 기반으로 기만적이므로 `source .env && printenv KEY`로 검증. 2026-09 발견: 32개 라인 prefix 유실로 homepage guard 실패
